...
;establece el color de las luces de los semaforos
to draw-stop-light
...
to go
  move-cars
  make-new-cars
  if auto? ;cambio de luces automático
...
to make-new-cars
  if (random-float 100 < freq-north) and not any? turtles-on patch 0 min-pycor
    [
      crt 1
        [ set ycor min-pycor
          ifelse(random-float 100 < freq-truck)
            [
              set shape "truck"
              set heading 0
              set color 5 + 10 * random 14
              set speed min (list clear-ahead speed-limit) * 0.5
            ]
            [
              set heading 0
              set color 5 + 10 * random 14
...
to move ;; turtle procedure
  ...
  [ if speed < speed-limit
    [ set speed speed + min (list max-accel (clear-to - 1 - speed)) ] ;acelera
    if speed > speed-limit
    [ set speed speed-limit ] ;establece velocidad al máximo
  ]
  [ set speed speed - min (list max-brake (speed - (clear-to - 1))) ;frena
  ...
  repeat speed ;mueve hacia adelante la cantidad correcta
  [
    fd 1
    if not can-move? 1
    [ die ]
    ...
    if pcolor = orange
    [ set clear-in 5  ;si choca contra un accidente, causa otro
      die
    ]
  ]
end
...
to-report clear-ahead ;;turtle procedure
  let n 1
  repeat max-accel + speed  ;determina el número de patches que se puede mover
