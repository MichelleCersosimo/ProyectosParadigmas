...
;; ------------------------------------------------------------------
    to crear-peatones  
      while [count peatones < numero-de-personas] [
...
to go
  ...
  ;; ==================================================
  ;; Parar la simulación cuando ya solo queda un carro
  ;; --------------------------------------------------
  if count turtles with [shape = "car"] = 1 and not any? turtles with [shape = "peaton"] [stop]
  ;;===================================================
...
;; --------------------------------------------------------------------------------------------------------
    to actualizar-hora-usuario
      ...
      ifelse ticks-por-hora != 0 [
        if ((ticks mod ticks-por-hora) = 0)
        [ set hora hora + 1 ]
      ]
      [
        show "ADVERTENCIA: Establezca un número para ticks-por-hora"
      ]
    end
;; ========================================================================================================
...
;; ----------------------------------------------------------------------
    to aumentar-trafico-predeterminada
      ...
      if ticks <= 165 [ set cantidad 5 ]                  ;; 5 - 6 - 7 - 9 
      if ticks > 165 and ticks <= 385 [ set cantidad 1 ]  ;; 9- 10 - 11 - 12
...
to set-car-speed  ;; turtle procedure
  ...
  ifelse [pcolor] of patch-ahead 1.5 = red
  [ set speed 0 ]
  [
    ifelse up-car?
    [ set-speed 0 -1 ]
