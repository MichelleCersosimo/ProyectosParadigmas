...
to setup-cars  ;; turtle procedure
  set speed 0.1 + random-float .9 ;; set initial speed to be in range 0.1 to 1.0
  set speed-min 0
  set wait-time 0
  put-on-empty-road
  ifelse intersection?
...
to kill-and-create 
  ...
  ifelse up-car?
  [  
    if any? turtles-on patch -7 min-pycor
    [ die ]
    if any? turtles-on patch 6 min-pycor
    [ die ]
    if any? turtles-on patch max-pxcor min-pycor
    [ die ]
  ]
  [  
    if any? turtles-on patch  max-pxcor 7
    [ die ]
    if any? turtles-on patch max-pxcor -5
    [ die ]
    if any? turtles-on patch max-pxcor min-pycor
    [ die ]
  ]  
...
to slow-down [turtles-ahead] ;; turtle procedure
  ifelse speed <= 0  ;;if speed < 0
  [ set speed 0 ]
  [ set speed [speed] of one-of turtles-ahead - deceleration ]
end
